...
const Size kGaussianBlueSize = Size(5, 5); //高斯模糊的大小
int main()
{
    VideoCapture video;
    video.open("C:/Users/Arlo/Downloads/Video/1.ts"); //获取视频
    Mat frame, channels[3], binary, Gaussian;
    vector<vector<Point>> contours; //储存轮廓
    vector<Vec4i> hierarchy; //轮廓层次
    Rect boundRect; //包围矩形，Rect创建二维矩形
    RotatedRect box;  //旋转矩形
    vector<Point2f> boxPts(4); //4个点
    ...
    for (;;) {
        ...
        split(frame, channels);//分离通道，放入channels
        threshold(channels[0], binary, kThreashold, kMaxVal, 0); //二值化，放入binary
        GaussianBlur(binary, Gaussian, kGaussianBlueSize, 0); //高斯滤波，放入Gaussian
        findContours(Gaussian, contours, hierarchy, RETR_TREE, CHAIN_APPROX_NONE);//找轮廓,放入contours
        int index = 0;
        //遍历所有轮廓
        ...
        try
        {
            ...
            for (int i = 0; i < 4; i++) {
                line(frame, p[i % 4], p[(i + 1) % 4], Scalar(0, 255, 0), 2); //绘制线段
            }
        }
        catch (const char* msg)
        {
